...
default:
   ...
   //$tampil=mysql_query("SELECT nasabah.no_rekening ,nama, sum(simpanan.nominal) where jenis='P' AS [pokok], sum(simpanan.nominal) from simpanan where jenis='S' AS [sukarela] FROM nasabah n INNER JOIN  simpanan s ON n.no_rekening=s.no_rek where no_rekening='$_GET[no_rekening]' $posisi,$batas");
    //$tampil=mysql_query("SELECT DISTINCTROW nasabah.no_rekening,nasabah.nama,(select sum(nominal) as pokok from simpanan where simpanan.jenis='P' and no_rek='$_GET[no_rekening]')as pokok,(select sum(nominal) as sukarela from simpanan where simpanan.jenis='S' and no_rek='$_GET[no_rekening]') as sukarela from nasabah inner join simpanan on nasabah. no_rekening=simpanan.no_rek where no_rekening='$_GET[no_rekening]' $posisi,$batas");
    $tampil=mysql_query("SELECT no_rekening,nama,sukarela, substring(no_rekening, 8, 3)AS urut FROM nasabah ORDER BY urut asc LIMIT $posisi,$batas");
   $no = $posisi+1;
