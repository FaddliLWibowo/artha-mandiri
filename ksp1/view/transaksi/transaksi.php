...
switch($_GET[act])
{
	...
	if (empty($_GET['rekening'])){
		...
		SUM(
			...
			ELSE 0
			END
			) AS `sukarela`, substring(no_rekening, 8, 3)AS urut from simpanan inner join nasabah on nasabah.no_rekening=simpanan.no_rek
		GROUP BY `no_rek` ORDER BY urut asc LIMIT $posisi,$batas");
		$no = $posisi+1;
		while($r=mysql_fetch_array($tampil)){
		    ...
		    echo "</table>";
		    $jmldata = mysql_num_rows(mysql_query("SELECT * FROM nasabah "));
			...
			<tr><td class='left'>no</td>
				<td class='left'>No Rekening</td>
				<td class='left'>Nama </td>
				<td class='left'>Pokoks </td>
				<td class='left'>Sukarela </td>
				<td class='center'>Menu</td>
			</tr></thead>";
	  ...
	  //<a href=?view=transaksi&act=transfer&no_rekening=$r[no_rekening]><img src='images/tranfer.png' border='0' title='Transfer' /></a>
			...
			<meta http-equiv='refresh' content='0; url=?view=transaksi&act=simpanan_pokok&no_rekening=$_GET[no_rekening]'>";	
				...
				$setoran = mysql_query(" select no_rekening, nama, SUM(
					CASE `jenis`
					WHEN 'P' THEN `nominal`
					ELSE 0
					END
				...
				SUM(
					...
					ELSE 0
					END
					) AS `sukarela` from nasabah n inner join simpanan s on n.no_rekening=s.no_rek  where no_rekening LIKE '%$_GET[no_rekening]%' ORDER BY cabang ");
				$r = mysql_fetch_array($setoran);

				echo "<h2 align='center'>SIMPANAN SUKARELA</h2>
				<br>
				<div class='nsb'>
					<form name='nasabah' id='nasabah' method=post action=$proses?view=transaksi&act=transaksi>
						<table class='listc'>
							<tbodyc>	
								...
								<tr><td class='a'>No Rekenings			</td><td class='b'> 	: $r[no_rekening]</td></tr>		 											
								...
								<tr><td class='a'>Sukarela			</td><td class='b'>		: ".num($r[sukarela])."</td></tr>
				...
				case"simpanan_pokok":
				$setoran = mysql_query("select no_rekening, nama, sum(nominal) as pokok, tanggal from nasabah n inner join simpanan s on n.no_rekening=s.no_rek where no_rekening='$_GET[no_rekening]' and jenis='P'");
				$r    = mysql_fetch_array($setoran);
					...
					<form name='nasabah' id='nasabah' method=POST action=$proses?view=transaksi&act=pokok >
						<table class='listc'>
							<tbodyc>
								...
								<tr><td class='a'>	Saldo Simpanan Pokokddd</td><td class='b'>  : $r[pokok] </td></tr>
