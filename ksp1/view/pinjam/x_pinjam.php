  	...
  	if ($_SESSION['jabatan']=='sa')
{
    ...
    $tampil=mysql_query("SELECT cabang,no_rek,no_pinjam,nama,alamat,jumlah,lama,tanggal,sisa,bunga From nasabah n inner join pinjam p on n.no_rekening = p.no_rek where status='Y' ORDER BY no_rekening asc LIMIT $posisi,$batas");
    $no=$posisi+1;
    while ($r=mysql_fetch_array($tampil)){
      echo "<tr><td class='center'>$no</td>
	  			<td class='center' width='50'>$r[no_pinjam]</td>
                <td class='center' width='80'>$r[no_rek]</td>
   ...
   break;
    ...
    $batas  = 4;
    $posisi = $p->cariPosisi($batas);
    $tampil=mysql_query("SELECT cabang,no_rek,no_pinjam,nama,alamat,jumlah,lama,tanggal,sisa,bunga From nasabah n inner join pinjam p on n.no_rekening = p.no_rek where status='Y' and  no_pinjam LIKE '%$_GET[no_pinjam]%' ORDER BY no_rekening asc LIMIT $posisi,$batas");
    $no=$posisi+1;
    while ($r=mysql_fetch_array($tampil)){
      echo "<tr><td class='center'>$no</td>
	  			<td class='center' width='50'>$r[no_pinjam]</td>
    ...
    $batas  = 4;
    $posisi = $p->cariPosisi($batas);
    $tampil=mysql_query("SELECT cabang,no_rek,no_pinjam,nama,alamat,jumlah,lama,tanggal,sisa,bunga From nasabah n inner join pinjam p on n.no_rekening = p.no_rek where cabang='$_SESSION[cab]' and status='Y' ORDER BY no_rekening asc LIMIT $posisi,$batas");
    $no=$posisi+1;
    while ($r=mysql_fetch_array($tampil)){
      echo "<tr><td class='center'>$no</td>
	  				<td class='center' width='50'>$r[no_pinjam]</td>
   ...
   break;
    ...
    $batas  = 4;
    $posisi = $p->cariPosisi($batas);
    $tampil=mysql_query("SELECT cabang,no_rekening,no_pinjam,nama,alamat,jumlah,lama,tanggal,sisa,bunga From nasabah n inner join pinjam p on n.no_rekening = p.no_rek where cabang='$_SESSION[cab]' and status='Y' and  no_pinjam LIKE '%$_GET[no_pinjam]%' ORDER BY no_rekening asc LIMIT $posisi,$batas");
    $no=$posisi+1;
    while ($r=mysql_fetch_array($tampil)){
      echo "<tr><td class='center'>$no</td>
	  				<td class='center' width='50'>$r[no_pinjam]</td>
  	...
  	if ($_SESSION['jabatan']=='sa')
{
	  ...
	  <div class='nsb'>
          <form name='nasabah' id='nasabah'  method=POST action='$proses?view=pinjam&act=pinjam' >
		   ...
		   echo "   <tr><td  class='a'>		Rekening Peminjam	</td><td 	class='b'>:  <select name='no_rek' id='no_rek'><option selected>- Masukan No Rekenings -</option>";
            $tampil=mysql_query("SELECT * from nasabah");
            while($z=mysql_fetch_array($tampil)){
           echo "<option value=$z[no_rek]> $z[no_rek] </option>";
            }
...
<br>
    ...
    $batas  = 5;
    $posisi = $p->cariPosisi($batas);
    $tampil=mysql_query("select cabang,no_pinjam,no_rek,nama,tanggal,jumlah,status,keterangan from pinjam p inner join nasabah n  on n.no_rekening=p.no_rek ORDER BY no_pinjam asc LIMIT $posisi,$batas");
...
<br>
    ...
    $batas  = 5;
    $posisi = $p->cariPosisi($batas);
    $tampil=mysql_query("SELECT cabang,no_pinjam,no_rek,nama,tanggal,jumlah,status,keterangan from pinjam p inner join nasabah n  on n.no_rekening=p.no_rek where cabang ='$_SESSION[cab]' ORDER BY no_pinjam asc LIMIT $posisi,$batas");
	...
	case"proses":
		$ok = mysql_query("select no_pinjam,no_rek,lama,nama,bunga,tanggal,jumlah,total,jenis,nama_jaminan,nama_pemilik,alamat_pemilik from pinjam p inner join nasabah n on n.no_rekening=p.no_rek where no_pinjam='$_GET[a]'");
          ...
          <form name='nasabah' id='nasabah'  method=POST action='$proses?view=pinjam&act=editpengajuan' >
          	<table class='listc'>
		  		<tbodyc>
          			...
          			<input type=hidden name='no_rek' id='no_rek' value='$r[no_rek]' readonly='true'> 
  ...
  case "angsur":
	...
	$sql 			= mysql_query(" select * from nasabah n inner join pinjam p on n.no_rekening=p.no_rek left join angsur a on p.no_pinjam=a.no_pnjm where no_pinjam='$_GET[no_pinjam]'");
...
<br>
    ...
    $batas  = 5;
    $posisi = $p->cariPosisi($batas);
    $tampil=mysql_query("select cabang,no_pinjam,no_rek,nama,tanggal,status from pinjam p inner join nasabah n  on n.no_rekening=p.no_rek where status='B' ORDER BY no_pinjam asc LIMIT $posisi,$batas");
    ...
    echo "</table>";
	$jmldata = mysql_num_rows(mysql_query(" select status from pinjam p inner join nasabah n on n.no_rekening=p.no_rek where status='B' "));
	$jmlhalaman  = $p->jumlahHalaman($jmldata, $batas);
    $linkHalaman = $p->navHalaman($_GET['halaman'], $jmlhalaman);
