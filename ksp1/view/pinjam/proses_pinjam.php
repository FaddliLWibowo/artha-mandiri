	...
	if (empty($_SESSION['nip']) AND empty($_SESSION['password'])){
		...
		if ($view=='pinjam' AND $act=='hapus'){
			...
			if ($cek!= "0") echo "<script>window.alert('Tidak Bisa Dihapus, Masih Memiliki Hutang')</script>	
				...
				$b		=mysql_query(	"UPDATE nasabah  	SET hutang				=	'N'	where no_rekening	= 	'$_GET[no_rekening]'") or die("n");							
		...
		elseif($view=='pinjam' AND $act=='pinjam'){
			...
			if($a) {echo "<script>window.alert('Silahkan Tunggu 1X24 Jam')</script>	
				<meta http-equiv='refresh' content='0; url=../../main.php?view=pinjam&act=cek_pengajuan'>";	
			}
			else
			{
		...
		elseif($view=='pinjam' AND $act=='prosespinjam'){
			...
			//echo "UPDATE pinjam SET status = '$_POST[status]', keterangan='$_POST[alamat_pemilik]',petugas='$_SESSION[nip]' where no_pinjam	= 	'$_POST[no_pinjam]'";

			$a=mysql_query(	"UPDATE pinjam SET status =	'$_POST[status]', keterangan='$_POST[alamat_pemilik]',petugas='$_SESSION[nip]' where no_pinjam	= 	'$_POST[no_pinjam]'") or die("p");	
