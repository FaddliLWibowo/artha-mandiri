...
if ($_SESSION['jabatan']=='sa')
{
    ...
    echo "<h1 align='center'>NASABAH</h1>
			<div>
    ...
    $tampil=mysql_query("SELECT  no_rekening,nama,alamat,no_identitas FROM nasabah ORDER BY no_rekening asc LIMIT $posisi,$batas");
    $no=$posisi+1;
    ...
    $batas  = 5;
    $posisi = $p->cariPosisi($batas);
    $tampil=mysql_query("SELECT no_rekening,nama,alamat,no_identitas FROM nasabah WHERE no_rekening LIKE '%$_GET[rekening]%' ORDER BY no_rekening asc LIMIT $posisi,$batas");
    $no=$posisi+1;
    while ($r=mysql_fetch_array($tampil)){
      echo "<tr><td class='center'>$no</td>
                <td class='left'>$r[no_rekening]</td>
 ...
 echo "<h2 align='center'>NASABAH</h2>
    ...
    $batas  = 5;
    $posisi = $p->cariPosisi($batas);
    $tampil=mysql_query("SELECT no_rekening,nama,alamat,no_identitas FROM nasabah where cabang='$cbg'  ORDER BY no_rekening asc LIMIT $posisi,$batas");
    $no=$posisi+1;
    while ($r=mysql_fetch_array($tampil)){
      echo "<tr><td class='center'>$no</td>
                <td class='left'>$r[no_rekening]</td>
 ...
 break;
    ...
    $batas  = 5;
    $posisi = $p->cariPosisi($batas);
    $tampil=mysql_query("SELECT no_rekening,nama,alamat,no_identitas FROM nasabah where cabang='$cbg' and no_rekening LIKE '%$_GET[rekening]%'  ORDER BY no_rekening asc LIMIT $posisi,$batas");
    $no=$posisi+1;
    while ($r=mysql_fetch_array($tampil)){
      echo "<tr><td class='center'>$no</td>
                <td class='left'>$r[no_rekening]</td>
...
break;
  ...
  case "editnasabah":
	...
	<div class='detailatas' align='center'><h1>PROFIL NASABAH<h1></div>
